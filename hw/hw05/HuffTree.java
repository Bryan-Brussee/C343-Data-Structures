...
// HuffTree class for CSCI C343 Summer 2016
//
...
public class HuffTree<Key, E> {
    ...
    private void getPath(BinNode<Integer, String> node, String code) {
	// base case, if we find a leaf, add it and its code to our code table.
        if (node.isLeaf() == true) {codeTable.put((String)node.getValue(), code);}
        // if we can go left, we should, and add a 0 to our path code.
	if (node.getLeft() != null) { getPath( node.getLeft(), code + "0" ); }
	// if we can go right, we should, and add a 1 to our path code.
	if (node.getRight() != null) { getPath( node.getRight(), code + "1" ); }
