...
public class BSTJr <K extends Comparable<?super K>> {
    ...
    public BinNode<K> smallestValue(BinNode<K> node, int k) {

	    BinNode<K> left = node.getLeft();
	    BinNode<K> right = node.getRight();
	    if (left == null && right == null) {return node;}
	
	    if (left.getSize() == k - 1) {return node;}
	    else if (left.getSize() > k -1 ) 
		{return smallestValue(left, k); }
	    else {return smallestValue(right, k - left.getSize() - 1);}
    }
	...
	tree.build(ks);
	tree.display();
	tree.smallestValue(tree.root, 4);
	//smallestValue(this.root, 7);
	System.out.println( tree.smallestValue(tree.root, 7).getKey() );
